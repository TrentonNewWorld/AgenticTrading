"""Futures Strategy Catalog (own roster, own cache, own registry -- not
merged into the stocks or options catalogs). Mirrors
test_options_strategy_catalog.py exactly. Backtester calls are stubbed (no
network); the starter roster's own decide_futures() code is exercised for
real through the sandbox, since that part needs no network access either.
"""
...
def _isolated_config(tmp_path, monkeypatch):
    ...
    src = catalog_module.LEADERBOARD_FUTURES_CONFIG_PATH
    dst = tmp_path / "leaderboard_futures.json"
    shutil.copy(src, dst)
    monkeypatch.setattr(catalog_module, "LEADERBOARD_FUTURES_CONFIG_PATH", dst)
...
def test_roster_matches_config():
    ...
    assert keys == ["fut_dip_reversion", "fut_donchian_breakout", "fut_zscore_meanrev", "fut_ema_ribbon", "fut_vol_breakout", "fut_trend_dip", "fut_multiday_momentum"]
...
def test_every_roster_entry_resolves_and_has_code():
    ...
    for entry in catalog_module._catalog_roster():
        ...
        assert "decide_futures" in strat.code()
...
def test_starter_strategies_are_not_in_the_stocks_registry():
    ...
    assert not any(k.startswith("fut_") for k in stocks_keys)


def test_starter_strategies_are_not_in_the_options_registry():
    from dashboard.backend.domain.options.strategies import available_strategies as options_strategies

    options_keys = set(options_strategies().keys())
    assert not any(k.startswith("fut_") for k in options_keys)
...
def test_generate_report_matches_stubbed_curve():
    report = catalog_module.generate_report("fut_dip_reversion")
    assert report["key"] == "fut_dip_reversion"
...
def test_build_export_returns_real_executable_code_for_every_starter():
    for key in ("fut_dip_reversion", "fut_donchian_breakout"):
        package = catalog_module.build_export(key)
        assert package["format"] == catalog_module.EXPORT_FORMAT
        assert "decide_futures" in package["code"]


def test_add_to_catalog_and_export_round_trips():
    code = "def decide_futures(as_of, positions, quotes, account):\n    return []\n"
    entry = catalog_module.add_to_catalog(name="My Futures Strat", description="test", code=code)
    assert entry["strategy"] == "fut_sandboxed"
...
def test_list_catalog_route(client):
    resp = client.get("/api/v1/futures/strategy-catalog")
...
def test_report_route(client):
    resp = client.get("/api/v1/futures/strategy-catalog/fut_dip_reversion/report")
    assert resp.status_code == 200, resp.text
    assert resp.json()["overall"]["final"] == pytest.approx(1100.0)


def test_report_route_unknown_key_is_404(client):
    resp = client.get("/api/v1/futures/strategy-catalog/nope/report")
    assert resp.status_code == 404


def test_export_route(client):
    resp = client.get("/api/v1/futures/strategy-catalog/fut_dip_reversion/export")
    assert resp.status_code == 200, resp.text
...
def test_delete_route(client):
    ...
    resp = client.delete(f"/api/v1/futures/strategy-catalog/{entry['id']}")
    assert resp.status_code == 200, resp.text
    assert resp.json() == {"removed": entry["id"]}


def test_delete_route_unknown_key_is_404(client):
    resp = client.delete("/api/v1/futures/strategy-catalog/nope")
    assert resp.status_code == 404
