...
_ROSTER = _pathlib.Path(__file__).resolve().parents[2] / "config" / "leaderboard_crypto.json"
if not _json.loads(_ROSTER.read_text(encoding="utf-8")).get("strategies"):
    _pytest.skip("blank-slate build: leaderboard_crypto.json has no strategies configured", allow_module_level=True)
