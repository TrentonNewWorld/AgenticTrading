"""Forex Strategy Catalog (own roster, own cache, own registry). Mirrors
test_futures_strategy_catalog.py exactly. Backtester calls are stubbed (no
network); the starter roster's own decide_forex() code is exercised for
...
def _isolated_config(tmp_path, monkeypatch):
    ...
    src = catalog_module.LEADERBOARD_FOREX_CONFIG_PATH
    dst = tmp_path / "leaderboard_forex.json"
    shutil.copy(src, dst)
    monkeypatch.setattr(catalog_module, "LEADERBOARD_FOREX_CONFIG_PATH", dst)
...
def test_starter_roster_has_two_entries():
    ...
    assert keys == ["fx_momentum_basket", "fx_dip_reversion", "fx_sma_cross", "fx_rsi_reversion", "fx_donchian_breakout", "fx_zscore_meanrev", "fx_ema_ribbon", "fx_vol_breakout", "fx_trend_dip", "fx_multiday_momentum"]
...
def test_every_roster_entry_resolves_and_has_code():
    ...
    for entry in catalog_module._catalog_roster():
        ...
        assert "decide_forex" in strat.code()
...
def test_starter_strategies_are_not_in_other_registries():
    ...
    for registry in (stocks_strategies(), options_strategies(), futures_strategies()):
        assert not any(k.startswith("fx_") for k in registry.keys())
...
def test_generate_report_matches_stubbed_curve():
    report = catalog_module.generate_report("fx_momentum_basket")
    assert report["key"] == "fx_momentum_basket"
...
def test_build_export_returns_real_executable_code_for_every_starter():
    for key in ("fx_momentum_basket", "fx_dip_reversion"):
        package = catalog_module.build_export(key)
        assert package["format"] == catalog_module.EXPORT_FORMAT
        assert "decide_forex" in package["code"]


def test_add_to_catalog_and_export_round_trips():
    code = "def decide_forex(as_of, positions, quotes, account):\n    return []\n"
    entry = catalog_module.add_to_catalog(name="My Forex Strat", description="test", code=code)
    assert entry["strategy"] == "fx_sandboxed"
...
def test_list_catalog_route(client):
    resp = client.get("/api/v1/forex/strategy-catalog")
...
def test_report_route(client):
    resp = client.get("/api/v1/forex/strategy-catalog/fx_dip_reversion/report")
    assert resp.status_code == 200, resp.text
    assert resp.json()["overall"]["final"] == pytest.approx(1100.0)


def test_report_route_unknown_key_is_404(client):
    resp = client.get("/api/v1/forex/strategy-catalog/nope/report")
    assert resp.status_code == 404


def test_export_route(client):
    resp = client.get("/api/v1/forex/strategy-catalog/fx_momentum_basket/export")
    assert resp.status_code == 200, resp.text
...
def test_delete_route(client):
    ...
    resp = client.delete(f"/api/v1/forex/strategy-catalog/{entry['id']}")
    assert resp.status_code == 200, resp.text
    assert resp.json() == {"removed": entry["id"]}


def test_delete_route_unknown_key_is_404(client):
    resp = client.delete("/api/v1/forex/strategy-catalog/nope")
    assert resp.status_code == 404
