"""Crypto Manual page: the multi-strategy manual-trading page's HTTP
surface for the Crypto dashboard. Mirrors api/routers/futures_manual.py and
api/routers/forex_manual.py exactly.
"""
...
from dashboard.backend.domain.crypto import engine, market_clock, repository as repo, uploads, views

router = APIRouter(prefix="/v1/crypto/manual", tags=["crypto-manual"])
...
def delete_strategy(key: str):
    if not repo.delete_uploaded_strategy(key):
        raise HTTPException(status_code=404, detail=f"no uploaded crypto strategy '{key}'")
    return {"deleted": key}
