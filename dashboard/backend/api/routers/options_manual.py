"""Options Manual page: the multi-strategy manual-trading page's HTTP
surface for the Options dashboard.

Sub-phase 10 of the Options-dashboard plan (Sub-phase 5 built the tick/
engine logic but no router for it -- this is that missing HTTP surface).
Mirrors ``api/routers/manual10.py``'s shape, simplified: Options has no
built-in Top-10-equivalent screener strategy (every strategy here is an
upload, per Sub-phase 5's design note), so there's no screener endpoint and
no promote/demote (no real-money path for Options in this phase -- see
``domain/options/engine.py``'s ``manual_sell`` docstring).
"""
...
from dashboard.backend.domain.options import engine, repository as repo, uploads, views

router = APIRouter(prefix="/v1/options/manual", tags=["options-manual"])
...
def delete_strategy(key: str):
    if not repo.delete_uploaded_strategy(key):
        raise HTTPException(status_code=404, detail=f"no uploaded options strategy '{key}'")
    return {"deleted": key}
