"""Forex Strategy Catalog HTTP surface. Mirrors
api/routers/futures_strategy_catalog.py exactly.
"""
...
from dashboard.backend.domain.forex.catalog import (
    build_export,
    generate_report,
...
router = APIRouter(prefix="/v1/forex/strategy-catalog", tags=["forex-strategy-catalog"])
...
def delete_catalog_strategy(key: str):
    ...
    if not removed:
        raise HTTPException(status_code=404, detail=f"no strategy '{key}' in the forex catalog")
    return {"removed": key}
