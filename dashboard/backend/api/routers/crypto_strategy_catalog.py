"""Crypto Strategy Catalog HTTP surface. Mirrors
api/routers/futures_strategy_catalog.py and
api/routers/forex_strategy_catalog.py exactly.
"""
...
from dashboard.backend.domain.crypto.catalog import (
    build_export,
    generate_report,
...
router = APIRouter(prefix="/v1/crypto/strategy-catalog", tags=["crypto-strategy-catalog"])
...
def delete_catalog_strategy(key: str):
    ...
    if not removed:
        raise HTTPException(status_code=404, detail=f"no strategy '{key}' in the crypto catalog")
    return {"removed": key}
