"""Futures Manual page: the multi-strategy manual-trading page's HTTP
surface for the Futures dashboard.

Mirrors api/routers/options_manual.py's shape exactly -- same reasoning: no
built-in screener strategy (every strategy here is an upload), so no
screener endpoint; no promote/demote (no real-money path armed by default
for Futures either -- see domain/futures/engine.py's manual_sell docstring).
Uses domain/futures/market_clock.py, not domain/manual10's -- futures trade
nearly 24/5, there is no NYSE session to key "today" off.
"""
...
from dashboard.backend.domain.futures import engine, market_clock, repository as repo, uploads, views

router = APIRouter(prefix="/v1/futures/manual", tags=["futures-manual"])
...
def delete_strategy(key: str):
    if not repo.delete_uploaded_strategy(key):
        raise HTTPException(status_code=404, detail=f"no uploaded futures strategy '{key}'")
    return {"deleted": key}
