"""Futures Strategy: every strategy in dashboard/config/
leaderboard_futures.json, presented as one card each. Mirrors
domain/options/catalog.py's shape exactly (own config file, own cache file,
same compute/cache/report/export functions) -- see that module's docstring
for the full reasoning (parallel module, not a shared registry, per the
user's decision that strategies never carry over between dashboards).
"""
...
from dashboard.backend.domain.futures.strategies import get_strategy
from dashboard.backend.paths import CONFIG_DIR, REPO_ROOT

CACHE_PATH = REPO_ROOT / "dashboard" / "storage" / "data" / "futures_strategy_catalog_cache.json"
LEADERBOARD_FUTURES_CONFIG_PATH = CONFIG_DIR / "leaderboard_futures.json"
CACHE_TTL_HOURS = 24
...
_DESCRIPTIONS: Dict[str, Dict[str, str]] = {
    "fut_momentum_basket": {"source": "Futures Strategy",
        "description": "Goes long any futures symbol in the default basket (equity index, metals, energy, rates) that closed above its previous close, and closes out on the first down day."},
    "fut_dip_reversion": {"source": "Futures Strategy",
        "description": "Buys any futures symbol in the default basket that fell 1%+ from its previous close, and closes out once it recovers back above that close."},
}
...
def _description_for(strategy_id: str, model_name: str, strat: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    if strat and strat.get("description"):
        return {"source": strat.get("source") or "Futures Strategy", "description": strat["description"]}
    return _DESCRIPTIONS.get(strategy_id) or {
        "source": "Futures Strategy",
        "description": f"{model_name} -- no description written yet.",
    }


def load_leaderboard_config() -> Dict[str, Any]:
    with LEADERBOARD_FUTURES_CONFIG_PATH.open(encoding="utf-8") as f:
        return json.load(f)


def _save_leaderboard_config(config: Dict[str, Any]) -> None:
    with LEADERBOARD_FUTURES_CONFIG_PATH.open("w", encoding="utf-8") as f:
...
def _unique_strategy_id(base_slug: str, config: Dict[str, Any]) -> str:
    ...
    candidate = f"fut_{base_slug}" if not base_slug.startswith("fut_") else base_slug
    n = 2
    while candidate in existing:
        candidate = f"fut_{base_slug}_{n}"
        n += 1
    return candidate
...
def add_to_catalog(*, name: str, description: str, code: str) -> Dict[str, Any]:
    ...
    entry: Dict[str, Any] = {
        ...
        "model": name,
        "strategy": "fut_sandboxed",
        "code": code,
        "symbols": [],
        "source": "Tested Upload",
        "description": description or f'Uploaded futures strategy "{name}", scanned and backtested over the '
...
def convert_agent_to_strategy(
    *, name: str, model_id: Optional[str], strategy_prompt: str, symbols: Optional[List[str]] = None,
    ...
    """Write a new Futures Strategy Catalog entry that trades exactly like the
    given My Agents futures agent's pipeline instruction, via the
    ``fut_llm_agent`` strategy (its ``strategy_prompt`` is replayed through
    ...
    entry: Dict[str, Any] = {
        ...
        "strategy": "fut_llm_agent",
        "strategy_prompt": prompt,
        "symbols": list(symbols) if symbols else [],
        "source": "My Agents",
        "description": f'Converted from the My Agents futures agent "{name}".',
    }
    config.setdefault("strategies", []).append(entry)
    _save_leaderboard_config(config)
    _append_placeholder_to_cache(entry)
...
def _catalog_roster() -> List[CatalogEntry]:
    ...
    for strat in config.get("strategies", []):
        ...
        entries.append(CatalogEntry(
            ...
            symbols=strat.get("symbols") or ["ES=F"],
        ))
    return entries
...
def generate_report(key: str) -> Dict[str, Any]:
    ...
    if entry is None:
        raise ValueError(f"no strategy '{key}' in the futures catalog")

    start_date, end_date = _current_window()
    strat = get_strategy(_config_for(entry))
    curve = strat.run(start_date, end_date, INITIAL_CAPITAL)
    if not curve:
        raise RuntimeError("no futures market data available (Yahoo Finance fetch failed for this strategy's symbol universe)")

    from dashboard.backend.domain.futures.report import build_report
...
def build_export(key: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "source": "futures_strategy_catalog",
    }
