"""Futures Manual page uploads: submission and review workflow.

Mirrors domain/options/uploads.py's submit/approve/reject shape exactly --
same reasoning (see that module's docstring): lands straight on the Manual
page for immediate selection once approved, no separate "Add to Catalog"
step. Futures has no built-in Top-10-equivalent screener strategy either
(the user's own decision for this dashboard's Manual page), so every Futures
Manual strategy is an upload.
"""
...
from dashboard.backend.domain.futures import repository as repo
from dashboard.backend.domain.futures.sandbox import FuturesStrategyCodeError, validate_code
...
_RISK_REVIEW_SYSTEM_PROMPT = """You are a security reviewer for a Python FUTURES trading strategy upload feature.
...
no credentials, no network, and a hard timeout. The function is decide_futures(as_of, positions, quotes, account),
...
doesn't look like a real futures trading strategy at all. Respond with a short plain-text assessment (2-4
...
def _llm_risk_review(code: str, user_id: Optional[int] = None) -> str:
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Review this uploaded futures strategy code:\n\n```python\n{code}\n```"}],
        )
...
def approve_upload(key: str) -> Dict[str, Any]:
    ...
    if strategy is None or strategy["kind"] != "uploaded":
        raise UploadError(f"no uploaded futures strategy '{key}'")
