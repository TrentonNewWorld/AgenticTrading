...
``asset_class='futures'`` -- same pattern as domain/options/repository.py
(see that module's docstring for the full reasoning: one schema, namespaced
keys, no widened primary keys). Simpler than the Options wrapper: futures
positions are single-instrument, so none of the contract-level columns
(strike/expiration/right/leg_group_id/leg_role) are ever set here.
"""
...
def list_activations(trading_date: str) -> List[Dict[str, Any]]:
    futures_keys = {s["key"] for s in list_strategies()}
    return [a for a in manual10_repo.list_activations(trading_date) if a["strategy_key"] in futures_keys]
...
def list_days(strategy_key: Optional[str] = None, limit: int = 90) -> List[Dict[str, Any]]:
    ...
    return [d for d in rows if d["strategy_key"] in futures_keys][:limit]
