"""Executes uploaded **futures** strategy code safely enough to auto-run
unattended.

Clones domain/options/sandbox.py's three-layer security model verbatim
(AST allowlist -> subprocess isolation with a stripped environment ->
restricted builtins) -- same reasoning: strategy logic is pure arithmetic
over data handed in, never given a broker client or credentials, regardless
of asset class.

Simpler contract than Options', on purpose -- the user chose an upload-only
Manual page for Futures (same as Options), but futures contracts are single
tradeable instruments, not multi-leg option chains, so there is no
strike/expiration/right/leg-grouping shape to carry:

    def decide_futures(as_of, positions, quotes, account) -> [order_intent, ...]

``as_of``: "YYYY-MM-DD".
``positions``: currently-held futures positions, each {symbol, qty}.
``quotes``: {symbol: {price, prev_close}} -- one flat dict, not an options-
  style chain, since a futures symbol has no strikes/expirations to fan out.
``account``: {cash, equity}.
...
class FuturesStrategyCodeError(ValueError):
...
def validate_code(code: str) -> None:
    try:
        ...
        raise FuturesStrategyCodeError(f"syntax error: {exc}") from exc

    issues: List[str] = []
    has_decide_futures = False

    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "decide_futures":
            has_decide_futures = True
    ...
    if not has_decide_futures:
        issues.append(
            "code must define a top-level function named "
            "decide_futures(as_of, positions, quotes, account)"
        )

    if issues:
        raise FuturesStrategyCodeError("; ".join(sorted(set(issues))))
...
def main():
    ...
    exec(compile(code, "<uploaded_futures_strategy>", "exec"), namespace)
    intents = namespace["decide_futures"](
...
def run_decide_futures(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="futures_sandbox_") as tmp:
        ...
        try:
            ...
            print("futures sandbox: strategy timed out")
            return []

        if result.returncode != 0:
            print(f"futures sandbox: strategy exited {result.returncode}: {result.stderr[-500:]}")
            return []

        try:
            raw = json.loads(result.stdout.strip().splitlines()[-1]) if result.stdout.strip() else []
        except (json.JSONDecodeError, IndexError):
            print(f"futures sandbox: strategy produced non-JSON output: {result.stdout[:200]!r}")
            return []
