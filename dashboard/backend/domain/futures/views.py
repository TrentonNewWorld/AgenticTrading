"""Read-only, display-oriented views over the Futures Manual page's ledger.

Mirrors domain/options/views.py's shape, simpler: every Futures position is a
single tradeable instrument priced from one flat Yahoo Finance quote (no
stock-leg-vs-option-leg distinction, no per-contract multiplier -- futures
"shares" here are simulated units of the underlying continuous contract, not
real point-value contracts, matching this dashboard's simulated-by-default
posture, see infrastructure/market_data/yahoo_futures.py's module docstring).
"""
...
from dashboard.backend.infrastructure.market_data.yahoo_futures import get_futures_quotes_batch


def _enrich(positions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    symbols = sorted({p["symbol"] for p in positions if p["status"] == "open"})
    try:
        quotes = get_futures_quotes_batch(symbols) if symbols else {}
    except Exception as exc:
        print(f"futures views: quote fetch failed: {exc}")
        quotes = {}
