...
``asset_class='forex'`` -- mirrors domain/futures/repository.py exactly
(see that module and domain/options/repository.py for the full reasoning).
"""
...
def list_activations(trading_date: str) -> List[Dict[str, Any]]:
    forex_keys = {s["key"] for s in list_strategies()}
    return [a for a in manual10_repo.list_activations(trading_date) if a["strategy_key"] in forex_keys]
...
def list_days(strategy_key: Optional[str] = None, limit: int = 90) -> List[Dict[str, Any]]:
    ...
    return [d for d in rows if d["strategy_key"] in forex_keys][:limit]
