"""Forex Strategy: every strategy in dashboard/config/
leaderboard_forex.json, presented as one card each. Mirrors
domain/futures/catalog.py exactly.
"""
...
CACHE_PATH = REPO_ROOT / "dashboard" / "storage" / "data" / "forex_strategy_catalog_cache.json"
LEADERBOARD_FOREX_CONFIG_PATH = CONFIG_DIR / "leaderboard_forex.json"
CACHE_TTL_HOURS = 24
...
_DESCRIPTIONS: Dict[str, Dict[str, str]] = {
    "fx_momentum_basket": {"source": "Forex Strategy",
        "description": "Goes long any major USD-quote pair (EUR/USD, GBP/USD, AUD/USD, NZD/USD) that closed above its previous close, and closes out on the first down day."},
    "fx_dip_reversion": {"source": "Forex Strategy",
        "description": "Buys any major USD-quote pair that fell 0.5%+ from its previous close, and closes out once it recovers back above that close."},
}
...
def _description_for(strategy_id: str, model_name: str, strat: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    if strat and strat.get("description"):
        return {"source": strat.get("source") or "Forex Strategy", "description": strat["description"]}
    return _DESCRIPTIONS.get(strategy_id) or {
        "source": "Forex Strategy",
        "description": f"{model_name} -- no description written yet.",
    }


def load_leaderboard_config() -> Dict[str, Any]:
    with LEADERBOARD_FOREX_CONFIG_PATH.open(encoding="utf-8") as f:
        return json.load(f)


def _save_leaderboard_config(config: Dict[str, Any]) -> None:
    with LEADERBOARD_FOREX_CONFIG_PATH.open("w", encoding="utf-8") as f:
...
def _unique_strategy_id(base_slug: str, config: Dict[str, Any]) -> str:
    ...
    candidate = f"fx_{base_slug}" if not base_slug.startswith("fx_") else base_slug
    n = 2
    while candidate in existing:
        candidate = f"fx_{base_slug}_{n}"
        n += 1
    return candidate
...
def add_to_catalog(*, name: str, description: str, code: str) -> Dict[str, Any]:
    ...
    entry: Dict[str, Any] = {
        ...
        "model": name,
        "strategy": "fx_sandboxed",
        "code": code,
        "symbols": [],
        "source": "Tested Upload",
        "description": description or f'Uploaded forex strategy "{name}", scanned and backtested over the '
...
def convert_agent_to_strategy(
    *, name: str, model_id: Optional[str], strategy_prompt: str, symbols: Optional[List[str]] = None,
    ...
    """Write a new Forex Strategy Catalog entry that trades exactly like the
    given My Agents forex agent's pipeline instruction, via the
    ``fx_llm_agent`` strategy (its ``strategy_prompt`` is replayed through
    ...
    entry: Dict[str, Any] = {
        ...
        "strategy": "fx_llm_agent",
        "strategy_prompt": prompt,
        "symbols": list(symbols) if symbols else [],
        "source": "My Agents",
        "description": f'Converted from the My Agents forex agent "{name}".',
    }
    config.setdefault("strategies", []).append(entry)
    _save_leaderboard_config(config)
    _append_placeholder_to_cache(entry)
...
def _catalog_roster() -> List[CatalogEntry]:
    ...
    for strat in config.get("strategies", []):
        ...
        entries.append(CatalogEntry(
            ...
            symbols=strat.get("symbols") or ["EURUSD=X"],
        ))
    return entries
...
def generate_report(key: str) -> Dict[str, Any]:
    ...
    if entry is None:
        raise ValueError(f"no strategy '{key}' in the forex catalog")

    start_date, end_date = _current_window()
    strat = get_strategy(_config_for(entry))
    curve = strat.run(start_date, end_date, INITIAL_CAPITAL)
    if not curve:
        raise RuntimeError("no forex market data available (Yahoo Finance fetch failed for this strategy's symbol universe)")

    from dashboard.backend.domain.forex.report import build_report
