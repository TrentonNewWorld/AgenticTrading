"""Day-by-day event-driven backtest for uploaded forex strategies, and for My
Agents' LLM-driven forex agents. Mirrors domain/futures/backtester.py's shape
exactly, including two things fixed there after a live-verification run and
baked in here from the start: quotes carry ``prev_close`` (yesterday's close,
read before today's price updates the running last-known-price map), and an
open is refused if its notional (price * qty) would exceed available cash, so
a position can never silently trade on unlimited implicit leverage against
the $1,000 wallet.
...
decision paths converge on ``_clean_intents`` (domain/forex/sandbox.py) for
...
from dashboard.backend.domain.forex.sandbox import _clean_intents, run_decide_forex
...
_LLM_AGENT_SYSTEM_PROMPT = """You are trading forex in a paper backtest for an agent with a fixed \
...
qty is a positive whole number of units. Return [] to do nothing this step.
...
def _run_daybyday(
    ...
    for symbol in symbols:
        try:
            bars = get_forex_daily_bars(symbol, start.isoformat(), end.isoformat())
    ...
    for day in trading_days:
        ...
        # prev_close read BEFORE last_known_prices is updated with today's
        # prices -- see module docstring.
        # quotes[sym]["closes"] (added 2026-08-29): trailing daily closes,
        # oldest -> newest, ending at today's price -- inside the quote dict
        # so the sandbox contract and existing strategies stay untouched.
        quotes = {}
...
def _llm_decide(client, model: Optional[str], prompt: str, as_of, positions, quotes, account) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"forex llm-agent backtest: decision failed on {as_of}: {exc}")
        return []
