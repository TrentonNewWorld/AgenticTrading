"""Options Strategy: every strategy in ``dashboard/config/
leaderboard_options.json``, presented as one card each (name, description,
own equity-curve chart, metrics).

Sub-phase 8 of the Options-dashboard plan. Mirrors ``domain/leaderboard/
catalog.py``'s shape (own config file, own cache file, same
compute/cache/report/export functions) but does **not** import from it or
extend its registry -- per the user's explicit decision, strategies do not
carry over between dashboards, so this is a parallel module reading its own
roster and its own strategy classes (``domain.options.strategies``).

Computing every strategy's full-year contract-level backtest on every page
load would be slow (each strategy probes/fetches real options history) and
would burn API quota on every visitor, so results are cached to a small JSON
file (``dashboard/storage/data/options_strategy_catalog_cache.json``) and
only recomputed when the cache is empty, stale, or a caller explicitly asks
for ``force_refresh`` -- same TTL convention as the stocks catalog.
"""
...
CACHE_PATH = REPO_ROOT / "dashboard" / "storage" / "data" / "options_strategy_catalog_cache.json"
LEADERBOARD_OPTIONS_CONFIG_PATH = CONFIG_DIR / "leaderboard_options.json"
CACHE_TTL_HOURS = 24
...
_DESCRIPTIONS: Dict[str, Dict[str, str]] = {
    "opt_covered_call_starter": {"source": "Options Strategy",
        "description": "Buys 100 shares of SPY, then sells 1 moderately out-of-the-money monthly call against them; rolls to a new call after each expiration."},
    "opt_cash_secured_put": {"source": "Options Strategy",
        "description": "Sells 1 moderately out-of-the-money monthly SPY put each cycle, collecting premium; rolls after each expiration."},
    "opt_long_call_momentum": {"source": "Options Strategy",
        "description": "Buys 1 near-the-money SPY call, holds to expiration, then re-enters -- the simplest single-leg, directional reference strategy."},
}
...
def _description_for(strategy_id: str, model_name: str, strat: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    if strat and strat.get("description"):
        return {"source": strat.get("source") or "Options Strategy", "description": strat["description"]}
...
def load_leaderboard_config() -> Dict[str, Any]:
    with LEADERBOARD_OPTIONS_CONFIG_PATH.open(encoding="utf-8") as f:
        return json.load(f)


def _save_leaderboard_config(config: Dict[str, Any]) -> None:
    with LEADERBOARD_OPTIONS_CONFIG_PATH.open("w", encoding="utf-8") as f:
...
def _unique_strategy_id(base_slug: str, config: Dict[str, Any]) -> str:
    ...
    candidate = f"opt_{base_slug}" if not base_slug.startswith("opt_") else base_slug
    n = 2
...
def add_to_catalog(*, name: str, description: str, code: str) -> Dict[str, Any]:
    """Promotes a 'ready' Testing-page options result into this catalog.
    Mirrors strategy_testing/promote.py's add_to_catalog, writing into
    leaderboard_options.json instead of the stocks leaderboard.json."""
    config = load_leaderboard_config()
    ...
    entry: Dict[str, Any] = {
        ...
        "model": name,
        "strategy": "opt_sandboxed",
        "code": code,
        "underlyings": [],
        "source": "Tested Upload",
        "description": description or f'Uploaded options strategy "{name}", scanned and backtested over the '
...
def convert_agent_to_strategy(
    *, name: str, model_id: Optional[str], strategy_prompt: str, symbols: Optional[List[str]] = None,
    ...
    """Write a new Options Strategy Catalog entry that trades exactly like the
    given My Agents options agent's pipeline instruction, via the
    ``opt_llm_agent`` strategy (its ``strategy_prompt`` is replayed through
    the LLM at every catalog compute). Mirrors
    ...
    ``symbols`` is accepted for signature parity with the other asset
    classes' convert functions (agents.py doesn't currently pass it for any
    of them) and is stored as this catalog's ``underlyings`` field."""
    ...
    entry: Dict[str, Any] = {
        ...
        "strategy": "opt_llm_agent",
        "strategy_prompt": prompt,
        "underlyings": list(symbols) if symbols else [],
        "source": "My Agents",
        "description": f'Converted from the My Agents options agent "{name}".',
    }
    config.setdefault("strategies", []).append(entry)
    _save_leaderboard_config(config)
    _append_placeholder_to_cache(entry)
...
def _catalog_roster() -> List[CatalogEntry]:
    ...
    for strat in config.get("strategies", []):
        ...
        entries.append(CatalogEntry(
            ...
            underlyings=strat.get("underlyings") or ["SPY"],
        ))
    return entries
...
def _config_for(entry: CatalogEntry) -> Dict[str, Any]:
    ...
    if strat is None:
        return {"id": entry.key, "name": entry.name, "strategy": entry.key, "underlyings": entry.underlyings}
...
def _current_window() -> tuple:
    """The most recent completed year -- domain.leaderboard.baselines'
    contest_window_for_year() is the single owner of this computation
    (shared with the Options Competition Leaderboard, Sub-phase 9), used
    here too so the catalog's own backtest window and the leaderboard's
    ranking window can never silently drift apart."""
...
def generate_report(key: str) -> Dict[str, Any]:
    ...
    if entry is None:
        raise ValueError(f"no strategy '{key}' in the options catalog")
    ...
    if not curve:
        raise RuntimeError(
            "no options market data available (check ALPACA_API_KEY/ALPACA_SECRET_KEY, "
            "or no listed contracts were found for this strategy's underlying universe)"
        )

    from dashboard.backend.domain.options.report import build_report
...
def build_export(key: str) -> Dict[str, Any]:
    """Unlike the stocks catalog (whose built-in strategies are Python
    classes with no standalone script to export), every Options strategy --
    built-in starter or user-uploaded -- is itself just a decide_options()
    source string (OptionsBaselineStrategy.code()), so every catalog entry
    here has real, re-runnable, exportable source."""
    config = load_leaderboard_config()
    ...
    return {
        ...
        "source": "options_strategy_catalog",
    }
