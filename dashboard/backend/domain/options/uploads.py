"""Options Manual page uploads: submission and review workflow.

Sub-phase 10 of the Options-dashboard plan. Mirrors ``domain/manual10/
uploads.py``'s submit/approve/reject shape for the Manual page's own direct
upload path (distinct from the Testing page's queue -- see that module's
docstring for why the two exist separately: this one lands straight on the
Manual page for immediate selection once approved, no separate "Add to
Catalog" step). Options has no built-in Top-10-equivalent screener strategy
(Sub-phase 5's design note), so every Options Manual strategy is an upload.
"""
...
from dashboard.backend.domain.options.sandbox import OptionsStrategyCodeError, validate_code
...
_RISK_REVIEW_SYSTEM_PROMPT = """You are a security reviewer for a Python OPTIONS trading strategy upload feature.
...
no credentials, no network, and a hard timeout. The function is decide_options(as_of, positions, chain, account),
...
doesn't look like a real options trading strategy at all. Respond with a short plain-text assessment (2-4
...
def _llm_risk_review(code: str, user_id: Optional[int] = None) -> str:
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Review this uploaded options strategy code:\n\n```python\n{code}\n```"}],
        )
...
def approve_upload(key: str) -> Dict[str, Any]:
    ...
    if strategy is None or strategy["kind"] != "uploaded":
        raise UploadError(f"no uploaded options strategy '{key}'")
