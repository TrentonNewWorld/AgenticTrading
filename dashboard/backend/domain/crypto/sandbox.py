"""Executes uploaded **crypto** strategy code safely enough to auto-run
unattended. Mirrors domain/futures/sandbox.py's and domain/forex/sandbox.py's
three-layer security model and contract shape, with one deliberate
difference: ``qty`` is a **float**, not an int.

    def decide_crypto(as_of, positions, quotes, account) -> [order_intent, ...]

``as_of``: "YYYY-MM-DD".
``positions``: currently-held crypto positions, each {symbol, qty}.
``quotes``: {symbol: {price, prev_close}} for the USD-quote major-coin
  universe (infrastructure/market_data/alpaca_crypto.py).
``account``: {cash, equity}.

Returns order intents: {action: "open"|"close", symbol, side: "buy"|"sell",
qty (positive float)}.

Futures and forex both use whole-unit int quantities (1 contract, 500
units) because a fractional futures contract or a fractional "500.5 units of
EUR" isn't a real, tradable size on those markets. Crypto is the opposite:
Alpaca confirmed every pair in this dashboard's universe is fractionable,
and a real position sized to fit a $1,000 wallet against BTC's ~$77,000
price is *necessarily* fractional (e.g. 0.01 BTC) -- rounding that up to a
whole coin would be a $77,000 order against a $1,000 wallet. Rejecting a
fractional qty here would make most of the universe untradeable at this
wallet size, not just an edge case.
"""
...
class CryptoStrategyCodeError(ValueError):
...
def validate_code(code: str) -> None:
    try:
        ...
        raise CryptoStrategyCodeError(f"syntax error: {exc}") from exc

    issues: List[str] = []
    has_decide_crypto = False

    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "decide_crypto":
            has_decide_crypto = True
    ...
    if not has_decide_crypto:
        issues.append(
            "code must define a top-level function named "
            "decide_crypto(as_of, positions, quotes, account)"
        )

    if issues:
        raise CryptoStrategyCodeError("; ".join(sorted(set(issues))))
...
def main():
    ...
    exec(compile(code, "<uploaded_crypto_strategy>", "exec"), namespace)
    intents = namespace["decide_crypto"](
...
def run_decide_crypto(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="crypto_sandbox_") as tmp:
        ...
        try:
            ...
            print("crypto sandbox: strategy timed out")
            return []

        if result.returncode != 0:
            print(f"crypto sandbox: strategy exited {result.returncode}: {result.stderr[-500:]}")
            return []

        try:
            raw = json.loads(result.stdout.strip().splitlines()[-1]) if result.stdout.strip() else []
        except (json.JSONDecodeError, IndexError):
            print(f"crypto sandbox: strategy produced non-JSON output: {result.stdout[:200]!r}")
            return []
