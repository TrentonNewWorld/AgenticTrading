"""Read-only, display-oriented views over the Crypto Manual page's ledger.
Mirrors domain/futures/views.py and domain/forex/views.py exactly.
"""
...
from dashboard.backend.infrastructure.market_data.alpaca_crypto import get_crypto_quotes_batch


def _enrich(positions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    symbols = sorted({p["symbol"] for p in positions if p["status"] == "open"})
    try:
        quotes = get_crypto_quotes_batch(symbols) if symbols else {}
    except Exception as exc:
        print(f"crypto views: quote fetch failed: {exc}")
        quotes = {}
