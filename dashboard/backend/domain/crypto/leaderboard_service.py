"""Crypto Competition + Live Trading Leaderboards -- ranked views of the
Crypto Strategy Catalog's own precomputed metrics. Mirrors
domain/futures/leaderboard_service.py and domain/forex/leaderboard_service.py
exactly -- see those modules and domain/options/leaderboard_service.py's
docstrings for the full reasoning (contest = most recently completed full
year, viewing-only; live = fixed-anchor forward-tracking window, not
derived from "today").
"""
...
LIVE_CACHE_PATH = REPO_ROOT / "dashboard" / "storage" / "data" / "crypto_live_leaderboard_cache.json"
...
def _compute_for_window(start_date: str, end_date: str) -> List[Dict[str, Any]]:
    entries_out = []
    for entry in crypto_catalog._catalog_roster():
        try:
            strat = get_strategy(crypto_catalog._config_for(entry))
            curve = strat.run(start_date, end_date, crypto_catalog.INITIAL_CAPITAL)
            metrics = crypto_catalog._metrics(curve)
            sampled = curve[::3] if len(curve) > 120 else curve
            if curve and sampled[-1] is not curve[-1]:
                sampled = sampled + [curve[-1]]
        except Exception as exc:
            metrics = {"final": crypto_catalog.INITIAL_CAPITAL, "return_pct": 0.0, "max_drawdown_pct": 0.0}
            sampled = []
...
def _live_season_start() -> Any:
    ...
    config = crypto_catalog.load_leaderboard_config()
...
def get_leaderboard(period: str = "contest", *, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    if period == "contest":
        ...
        catalog_payload = crypto_catalog.get_strategy_catalog(force_refresh=force_refresh)
        return {
            "period": "contest",
