"""Crypto Manual page uploads: submission and review workflow. Mirrors
domain/futures/uploads.py and domain/forex/uploads.py exactly.
"""
...
from dashboard.backend.domain.crypto.sandbox import CryptoStrategyCodeError, validate_code
...
_RISK_REVIEW_SYSTEM_PROMPT = """You are a security reviewer for a Python CRYPTO trading strategy upload feature.
...
no credentials, no network, and a hard timeout. The function is decide_crypto(as_of, positions, quotes, account),
...
doesn't look like a real crypto trading strategy at all. Respond with a short plain-text assessment (2-4
...
def _llm_risk_review(code: str, user_id: Optional[int] = None) -> str:
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Review this uploaded crypto strategy code:\n\n```python\n{code}\n```"}],
        )
...
def approve_upload(key: str) -> Dict[str, Any]:
    ...
    if strategy is None or strategy["kind"] != "uploaded":
        raise UploadError(f"no uploaded crypto strategy '{key}'")
